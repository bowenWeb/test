...
export default {
  ...
  computed: {
    ...
    allGasFee () {
      const gas = this.formData.gasFeeCap * this.formData.gasLimit / Math.pow(10, Number(9))
      return gas || 0
    },
    gasUnit () {
      let unit = ''
    ...
    total () {
      ...
      const big = new BigNumber(total).toFixed()
      const str = formatNumber(big, 12)
      return str
    },
    active () {
        ...
        if (this.mainBalance > this.allGasFee) {
          if (this.formData.isAll) {
            if (this.formData.isMain === 1) {
              const val = this.formData.balance - this.allGasFee
